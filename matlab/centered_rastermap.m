...
% THIS NEEDS TO DERIVE NEW GRADIENTS FOR CENTER SURROUND GAUSSIAN BASIS
% FUNCTIONS
% eta = eta/10;
tic
for k = 1:niter
...
drawnow
%%
irand = randperm(NN, min(NN, 1e3));
ds = reshape(ys(irand, :), [numel(irand), 1, ndims]) - reshape(ys, [1, NN, ndims]);
...
X0 = gpuArray.zeros(numel(irand), size(Xz,2), 'single');
cb = zeros(256,1);
for j = 1:256
    X0 = X0 + Xz(isort(:, j),:);
    X0z = zscore(X0, [], 2)/size(X0,2)^.5;
    cc = sum(Xz(irand, :) .* X0z, 2);
    cb(j) = gather(mean(cc));
end
