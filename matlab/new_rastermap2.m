...
for k = 1:niter
    ...
    for j = 1:ndims
       S = S .* sin(pi/4 + ys(:,j) * ks(1:nc(k),j)');
    end
    for j = 1:ndims
        dS(:,:,j) = S  ./ sin(pi/4 + ys(:,j) * ks(1:nc(k),j)');
        dS(:,:,j) = dS(:,:,j) .* cos(pi/4 + ys(:,j) * ks(1:nc(k),j)') .* ks(1:nc(k),j)';
    end    
    ...
    lam = mean(ypred .* U, 2) ./ (1e-8 + mean(ypred.^2, 2));        
    err = lam .* ypred - U;
    if rem(k,25)==1
    ...
    if rem(k,100)==1
        fprintf('iter %d, eta %d, time %2.2f, err %2.6f \n', k, eta(k),  toc, cnew)
    end
