...
class Rastermap:
    ...
    def _map(self, X, dims):
        NN,nPC = X.shape
        tic = time.time()

        Uz = zscore(X, axis=1)/X.shape[1]**.5
        CC = Uz @ Uz.T

        CCsort = np.sort(CC, axis=0)[::-1, :]
        M = (CC > CCsort[400,:]).astype('float32')
        M = M - np.diag(np.diag(M))
        #M = np.ones(NN)

        #R = np.zeros(CC.shape)
        Y,b,c = svdecon(CC, k=2)
        R = (Y * b) @ c.T
        ...
        for t in range(niter):
            M0 = CC * M + R * (1.-M)
            u = np.ones(M0.shape[0])
            u /= np.sum(u**2)**.5
            u = u[:,np.newaxis]
            Msub = u @ (u.T @ M)
            Y, b, c = svdecon(M0 - Msub, k=2)
            R = (Y * b) @ c.T + Msub
            E[t] = 1 - np.sum(((CC - R)*M)**2) / Cnorm
            if t%10==0:
