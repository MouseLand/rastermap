...
class Rastermap:
    ...
    def _map(self, X, dims, nclust, u, ikeep, SALL=None):
        if self.mode is 'parallel':
            Xall = X
            X = Xall[1]
        ...
        xid = np.zeros(NN0)
        for j in range(dims):
            iclust = np.floor(nclust* (.5 + 1 * (u[:,j].astype(np.float64)/NN0 - .5)))
        ...
        #nkeep = NN0 * 2**np.arange(0, np.log2(NN/NN0), 0.2)
        nkeep = np.linspace(NN0, NN, 50).astype('int')
        #nkeep = NN0 * 2**np.arange(0, np.log2(NN/NN0), 0.2)
        phase0 = NN0 * np.ones(10, 'int')
        phaseX = NN * np.ones(10, 'int')
        nkeep = np.hstack((phase0, nkeep, phaseX)).astype('int')
        ...
        S = SALL
        xopt = []
        iK = np.zeros((len(nkeep), NN), 'bool')
        for t,nc in enumerate(nkeep):
            # get basis functions
            ...
            eweights = np.zeros((NN, npix))
            eweights[ikeep, :] = ((S.T / nA) @ S)[xid, :] * lam[:, np.newaxis]
            AtS     = A.T @ S
            ...
            ineusort = np.argsort(cmax2)[::-1]
            ikeep = ineusort[:nkeep[t]]
            ikeep = np.sort(ikeep)
            cmap = cmap[ikeep]
            cmax = cmax[ikeep]
            vnorm = vnorm[ikeep]
            ...
            xid     = np.argmax(cmap, axis=1)
            lam    = np.sqrt(cmax / vnorm[np.arange(len(xid)), xid])
            #lam    = np.sqrt(cmax / vnorm[0, xid])

            ft = .8 * np.sqrt(len(ikeep)/NN)
            xid,xs  = shrink_to_center(cmap, dims, nclust, 10, ft)
            xopt.append(xs)
            iK[t][ikeep] = True

            E[t]    = np.nanmean(cmax)/np.nanmean(xnorm[ikeep])
            if self.mode is 'parallel':
                cmapx[t%2] = cmap
            if t%10==0:
                if self.verbose:
