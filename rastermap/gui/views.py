...
class PlaneWindow(QMainWindow):
    ...
    def update_plots(self, roi_id=None):
        for ii in range(self.nY):
            for jj in range(self.nX):
                iplane = ii*self.nX + jj
                ip = self.z==iplane
                self.plot_scatter(self.x[ip], self.y[ip], iplane=iplane, neurons=ip, roi_id=roi_id)
    ...
    def neurons_selected(self, selected=None, neurons=None):
        ...
        neurons_select[self.sorting[selected.start * self.smooth_bin : 
                                    selected.stop * self.smooth_bin]] = True
            ...
            if roi_id is None:
                ...
                for roi_id in range(nclust_max):
                    if roi_id < len(self.cluster_rois):
                        selected = self.neurons_selected(self.cluster_slices[roi_id], neurons=neurons)
