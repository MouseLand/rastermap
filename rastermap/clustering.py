...
def kmeans_init(X, n_clusters=100, n_local_trials=None):
    ...
    for c in range(1, n_clusters):
        ...
        distance_to_candidates = 1 - (X @ X_candidates.T) / np.outer(X_norm, X_candidates_norm)
...
def scaled_kmeans(X, n_clusters=100, n_iter=50, 
    ...
    if X_nodes.shape[0] < n_clusters and init!='kmeans++':
        warnings.warn('found fewer clusters than user specified, try reducing n_clusters and/or reduce n_splits and/or increase n_PCs')
